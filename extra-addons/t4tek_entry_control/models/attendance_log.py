...
class EntryControlAttendanceLog(models.Model):
    ...
    def _find_existing_log(self, controller, device, serial_number, employee, employee_code_controller, check_time, check_type, verify_type, controller_local_id=False):
        ...
        if employee:
            ...
            # If Odoo cannot map the Controller employee id to hr.employee yet,
            # keep duplicate detection isolated by the raw Controller code.
            # Otherwise two unknown employees on the same device/time could collapse
            # into one Attendance Log.
            domain.extend([
                ("employee_id", "=", False),
                ("employee_code_controller", "=", employee_code_controller or ""),
            ])
    ...
    def ingest_direct_log(self, controller, data):
        ...
        api_employee_id = str(
            ...
            or data.get("enroll_number")
            or data.get("enrollNumber")
            or data.get("user_id")
            or data.get("userId")
            or ""
        ).strip()
