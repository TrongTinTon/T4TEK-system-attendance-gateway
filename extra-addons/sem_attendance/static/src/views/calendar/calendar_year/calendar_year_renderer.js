...
export class CalendarYearRenderer extends Component {
    ...
    static template = "sem_attendance.CalendarYearRenderer";
    static props = {
        model: Object,
    ...
    get options() {
        return {
            ...
            unselectAuto: false,
            weekNumberCalculation: (date) => getLocalYearAndWeek(this.model.rangeStart).week,
            weekNumbers: false,
    ...
    openPopover(target, date, records) {
        this.popover.open(target, this.getPopoverProps(date, records), "o_attendance_cw_popover");
    }
    unselect() {
    ...
    getDayCellClassNames(info) {
        ...
        if (this.props.model.unusualDays.includes(date)) {
            return ["o_attendance_calendar_disabled"];
        }
        return [];
    }
    eventClassNames({ event }) {
        ...
        if (record) {
            ...
            if (typeof color === "number") {
                classesToAdd.push(`o_attendance_calendar_color_${color}`);
            } else if (typeof color !== "string") {
                classesToAdd.push("o_attendance_calendar_color_0");
            }
